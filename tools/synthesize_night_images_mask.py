...
# ==============================================================================
# 1. 원본 데이터셋의 최상위 폴더 경로 (예: /home/sjy/dataset/Test)
#    이 폴더 바로 아래에 arctic_fox, black_cat_1 등의 폴더가 있어야 합니다.
MOCA_ROOT_PATH = "/home/sjy/paper/JED-VCOD/data/MoCA-Mask/Train"

# 2. 변환된 이미지가 저장될 최상위 폴더 경로
OUTPUT_ROOT_PATH = "/home/sjy/paper/JED-VCOD/data/MoCA-Mask/Train_Night"
# ==============================================================================


def main():
    """
    데이터셋의 'Imgs' 폴더에 있는 주간 이미지를 InstructPix2Pix를 사용해 야간 이미지로 변환합니다.
    'GT' 폴더는 건너뜁니다.
    """
    # 1. GPU 사용 가능 여부 확인
    ...
    # 3. 변환할 이미지 목록 탐색 (수정된 부분)
    # MOCA_ROOT_PATH 아래의 모든 하위 폴더에서 'Imgs'라는 이름의 폴더 안에 있는 .jpg 파일을 찾습니다.
    # 이렇게 하면 'GT' 폴더는 자동으로 제외됩니다.
    source_image_pattern = os.path.join(MOCA_ROOT_PATH, "**", "Imgs", "*.jpg")
    source_image_paths = glob.glob(source_image_pattern, recursive=True)
    
    if not source_image_paths:
        print(f"오류: '{MOCA_ROOT_PATH}' 경로의 'Imgs' 폴더들에서 이미지를 찾을 수 없습니다. 경로를 확인해주세요.")
        return
