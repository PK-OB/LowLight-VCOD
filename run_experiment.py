# run_experiment.py
# (전략 1: DCNetStyleVCOD 모델 적용)
# (전략 2: Boundary Loss 적용)
...
def verify_and_save_samples(dataset, common_cfg, train_cfg):
    """
    데이터셋 샘플을 이미지로 저장하여 로딩 확인 (Multi-Task 데이터셋 반영 버전, 변경 없음)
    """
...
# ▼▼▼ [전략 2] boundary_loss 인자 추가 ▼▼▼
def train_one_epoch(model, raft_model, raft_transforms, train_loader, optimizer, device, focal_loss, dice_loss, l1_loss, boundary_loss, writer, epoch, train_cfg, common_cfg):
    """
    한 에포크 학습 로직 (Boundary Loss 추가)
    """
    model.train()
    epoch_loss = 0.0
    ...
    for i, batch in enumerate(progress_bar):
        ...
        try:
            ...
            if train_cfg.get('lambda_boundary', 0) > 0:
                loss_boundary = boundary_loss(predicted_masks_flat, masks_flat_target)
                if torch.isnan(loss_boundary) or torch.isinf(loss_boundary): 
                    logging.warning(f"E{epoch+1} B{i}: Boundary Loss is NaN/Inf, setting to 0.")
                    loss_boundary = torch.tensor(0.0).to(device)
            # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲

            # ▼▼▼ [전략 2] 총 손실 (Boundary Loss 포함) ▼▼▼
            total_loss = loss_seg + \
                         train_cfg.get('lambda_temporal', 0.5) * loss_temporal + \
                         train_cfg.get('lambda_enhancement', 0.5) * loss_enhancement + \
                         train_cfg.get('lambda_boundary', 0.5) * loss_boundary # <-- 추가
            # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲

            if torch.isnan(total_loss) or torch.isinf(total_loss):
                # ▼▼▼ [전략 2] 로깅 업데이트 ▼▼▼
                logging.error(f"!!! E{epoch+1} B{i}: Total Loss is NaN/Inf after combining. Seg:{loss_seg.item():.4f}, Enh:{loss_enhancement.item():.4f}, Temp:{loss_temporal.item():.4f}, Bnd:{loss_boundary.item():.4f}. Skip backward.")
                # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
        ...
        if (i + 1) % accumulation_steps == 0:
            try:
                ...
                # ▼▼▼ [전략 2] 경계 손실 로깅 추가 ▼▼▼
                writer.add_scalar('Loss/Step/Boundary', loss_boundary.item(), global_step)
                # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
        ...
        try:
            if 'predicted_masks_seq' in locals() and predicted_masks_seq.nelement() > 0 and \
               ...
               'video_clip' in locals() and video_clip.nelement() > 0:
                ...
                b_last, t_last, c_last, h_last, w_last = video_clip.shape
                idx_to_save = 0 
                frame_to_save = 0
...
# ▼▼▼ [전략 2] boundary_loss 인자 추가 ▼▼▼
def validate(model, val_loader, device, focal_loss, dice_loss, l1_loss, boundary_loss, train_cfg):
    """
    검증 로직 (Boundary Loss 추가)
    """
    model.eval()
    val_loss = 0.0
    val_seg_loss = 0.0
    val_enhance_loss = 0.0
    val_boundary_loss = 0.0 # <-- Boundary loss 누적
    num_batches = 0
    ...
    with torch.no_grad():
        ...
        for batch in progress_bar:
            ...
            try:
                ...
                # ▼▼▼ [전략 2] 총 손실 (Boundary Loss 포함) ▼▼▼
                # (Validation에서는 Temporal Loss 제외)
                loss = loss_seg + \
                       train_cfg.get('lambda_enhancement', 0.5) * loss_enhancement + \
                       train_cfg.get('lambda_boundary', 0.5) * loss_boundary # <-- 추가
                # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
                ...
                if not torch.isnan(loss) and not torch.isinf(loss):
                    ...
                    val_boundary_loss += loss_boundary.item() # <-- 누적
                    num_batches += 1
    ...
    # ▼▼▼ [전략 2] 평균 경계 손실 계산 ▼▼▼
    avg_boundary_loss = val_boundary_loss / num_batches if num_batches > 0 else 0.0
    # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
    
    # ▼▼▼ [전략 2] 4개 값 반환 ▼▼▼
    return avg_total_loss, avg_seg_loss, avg_enhance_loss, avg_boundary_loss
    # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
...
def main(args): 
    ...
    try:
        model = DCNetStyleVCOD(
            ...
            gru_hidden_dim=train_cfg.get('gru_hidden_dim', 128), # <-- cfg에서 읽도록 수정
            decoder_channel=train_cfg.get('decoder_channel', 64), # <-- cfg에서 읽도록 수정
    ...
    # ▼▼▼ [전략 2] 손실 함수 (BoundaryLoss 추가 및 device로 이동) ▼▼▼
    focal_loss = FocalLoss().to(device)
    dice_loss = DiceLoss().to(device)
    l1_loss = nn.L1Loss().to(device)
    boundary_loss = BoundaryLoss().to(device) # <-- BoundaryLoss 초기화
    # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
    ...
    try:
        ...
        split_ratio = train_cfg.get('train_val_split_ratio', 0.8) 
    ...
    logger.info(f"Dataset split confirmed: {len(train_dataset)} train, {len(val_dataset)} val.")
    ...
    for epoch in range(start_epoch, epochs): 
        # Train
        # ▼▼▼ [전략 2] boundary_loss 전달 ▼▼▼
        train_loss = train_one_epoch(model, raft_model, raft_transforms, train_loader, optimizer, device, focal_loss, dice_loss, l1_loss, boundary_loss, writer, epoch, train_cfg, common_cfg)
        if train_loss < 0: logger.error(f"Epoch {epoch+1}: Train stopped due to critical error (NaN/Inf)."); break 
            
        # Validate
        # ▼▼▼ [전략 2] boundary_loss 전달 및 val_boundary_loss 받기 ▼▼▼
        val_loss, val_seg_loss, val_enhance_loss, val_boundary_loss = validate(model, val_loader, device, focal_loss, dice_loss, l1_loss, boundary_loss, train_cfg)
        
        # 로깅
        lr_log = optimizer.param_groups[0]['lr']
        # ▼▼▼ [전략 2] 로깅에 Boundary Loss 추가 ▼▼▼
        logger.info(f"Epoch {epoch+1}/{epochs} | Train Loss: {train_loss:.4f} | Val Total Loss: {val_loss:.4f} (Seg: {val_seg_loss:.4f}, Enh: {val_enhance_loss:.4f}, Bnd: {val_boundary_loss:.4f}) | LR: {lr_log:.6f}")
        ...
        writer.add_scalar('Loss/Epoch/Val_Boundary', val_boundary_loss, epoch) # <-- 추가
        writer.add_scalar('Learning_Rate', lr_log, epoch)
        # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲

        # Scheduler Step (변경 없음)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="JED-VCOD Training Script with DCNetStyleVCOD and Boundary Loss") # <-- 설명 변경
